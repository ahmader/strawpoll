...
class Main:
    ...
    # GLOBAL VARIABLES
    proxyListFile = "proxy1.xml"
    saveStateFile = "state1.xml"
    proxyTimeout = 25 # Seconds
    ...
    def sendToWebApi(self, httpsProxy):
        try:
            ...
            payload = {'content_id': self.surveyId, 'checked_answers': self.voteFor, 'name': '', 'token': ''}
            proxyDictionary = {
                # "http": 'http://' + httpsProxy
                "https": 'https://' + httpsProxy
            }
            ...
            # Connect to server
            r = requests.post('https://strawpoll.com/api/poll/vote', json=payload, headers=headers, proxies=proxyDictionary, timeout=self.proxyTimeout)
            json = r.json()
